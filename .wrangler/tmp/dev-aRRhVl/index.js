...
var MOXFIELD_API = "https://api.moxfield.com";
var MOXFIELD_HEADERS = {
...
__name(parseMoxfieldBody, "parseMoxfieldBody");
__name2(parseMoxfieldBody, "parseMoxfieldBody");
async function fetchMoxfieldViaCurl(path) {
  ...
  try {
    ...
    if (stderr2) {
      return {
        status: 500,
        body: { error: "curl_error", detail: stderr2 }
      };
    }
    return {
      status: 200,
      body: parseMoxfieldBody(stdout2)
    };
  } catch (error3) {
    return {
      status: 500,
      body: { error: "exec_failed", detail: error3.message }
    };
  }
}
__name(fetchMoxfieldViaCurl, "fetchMoxfieldViaCurl");
__name2(fetchMoxfieldViaCurl, "fetchMoxfieldViaCurl");
async function fetchMoxfieldViaFetch(path) {
  ...
  try {
    ...
    return {
      status: 500,
      body: { error: "fetch_failed", detail: error3.message }
    };
  }
}
__name(fetchMoxfieldViaFetch, "fetchMoxfieldViaFetch");
__name2(fetchMoxfieldViaFetch, "fetchMoxfieldViaFetch");
async function fetchMoxfield(path) {
  const fetched = await fetchMoxfieldViaFetch(path);
  const body = fetched.body;
  const canUseCurlFallback = typeof process !== "undefined" && process.versions?.node && !process.env.VERCEL && !process.env.CF_PAGES;
  if (canUseCurlFallback && typeof body === "object" && body && "error" in body && (body.error === "invalid_json" || body.error === "fetch_failed")) {
    return fetchMoxfieldViaCurl(path);
  }
  return fetched;
}
__name(fetchMoxfield, "fetchMoxfield");
__name2(fetchMoxfield, "fetchMoxfield");
async function handleMoxfieldUserDecksRequest(username, pageNumber = "1", pageSize = "100") {
...
__name(handleMoxfieldUserDecksRequest, "handleMoxfieldUserDecksRequest");
__name2(handleMoxfieldUserDecksRequest, "handleMoxfieldUserDecksRequest");
async function handleMoxfieldDeckRequest(publicId) {
...
__name(handleMoxfieldDeckRequest, "handleMoxfieldDeckRequest");
__name2(handleMoxfieldDeckRequest, "handleMoxfieldDeckRequest");
async function handleMoxfieldProxyRequest(url) {
  if (!url) {
...
__name(handleMoxfieldProxyRequest, "handleMoxfieldProxyRequest");
__name2(handleMoxfieldProxyRequest, "handleMoxfieldProxyRequest");
function textResponse(result, extraHeaders = {}) {
  const headers = {
...
function jsonResponse(result, extraHeaders = {}) {
  const headers = {
...
function binaryResponse(result, extraHeaders = {}) {
  const headers = {
    "Access-Control-Allow-Origin": "*",
    ...extraHeaders
  };
...
__name(binaryResponse, "binaryResponse");
__name2(binaryResponse, "binaryResponse");
...
__name2(readRequestBody, "readRequestBody");
...
async function handleScryfallProxyRequest({ url, method = "GET", body }) {
  if (!url) {
...
__name(handleScryfallProxyRequest, "handleScryfallProxyRequest");
__name2(handleScryfallProxyRequest, "handleScryfallProxyRequest");
...
var UPSTREAM = "https://mtg-assistant.up.railway.app/decks/analyze-complete";
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ...
        detail: error3 instanceof Error ? error3.message : "Failed to reach CommanderBracket."
      }),
      {
        status: 502,
        headers: {
...
var ALLOWED_HOST_SUFFIXES = ["wasabisys.com", "mtgcardbuilder.com", "wp.com"];
...
__name2(isAllowedImageHost, "isAllowedImageHost");
var IMAGE_PATH_EXT = /\.(png|jpe?g|gif|webp|bmp|svg)(\?|$)/i;
function upstreamLooksLikeImage(contentType, pathname) {
  if (contentType.startsWith("image/")) return true;
  return IMAGE_PATH_EXT.test(pathname);
}
__name(upstreamLooksLikeImage, "upstreamLooksLikeImage");
__name2(upstreamLooksLikeImage, "upstreamLooksLikeImage");
async function handleImageProxyRequest(uri) {
  if (!uri) {
...
__name(handleImageProxyRequest, "handleImageProxyRequest");
__name2(handleImageProxyRequest, "handleImageProxyRequest");
async function onRequest4(context2) {
  const url = new URL(context2.request.url);
  const result = await handleImageProxyRequest(url.searchParams.get("uri") ?? void 0);
  if (result.headers) {
...
var routes = [
  {
    routePath: "/api/moxfield/:path*",
    mountPath: "/api/moxfield",
    method: "",
    middlewares: [],
    modules: [onRequest]
  },
  {
    routePath: "/api/scryfall/:path*",
    mountPath: "/api/scryfall",
    method: "",
    middlewares: [],
    modules: [onRequest2]
  },
  {
    routePath: "/api/commander-bracket",
    mountPath: "/api",
    method: "",
    middlewares: [],
    modules: [onRequest3]
...
// .wrangler/tmp/bundle-3aIuSn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3aIuSn/middleware-loader.entry.ts
